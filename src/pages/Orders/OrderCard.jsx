...
const OrderCard = ({ order, onClick }) => {
    ...
    return (
        <div onClick={onClick} style={{ backgroundColor: order?.orderType === 'delivery' ? theme.palette.primary.main : theme.palette.secondary.main }} className='order_card'>
            <div className="order_card_top">
                <Typography variant='h5'>Order #{order?.orderNumber}</Typography>
                {order?.orderType === 'delivery' ? <LocalShippingIcon /> : <DirectionsWalkIcon />}
            ...
            <div className="order_card_bottom">
                ...
                <Typography>Order Type: {order?.orderType === 'delivery' ? 'Delivery' : 'Pick Up'}</Typography>
                {order?.orderType === 'delivery' && (<Typography> Address: {order?.customer.address.street}, {order?.customer.address.zipcode}</Typography>)}
